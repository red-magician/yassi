...
# ポータルの「受賞者」シート形式（04_受賞者マスタとは列名が異なり、氏名は日本語表記＋「さん」付き、
# EntraId等の一意キーは無い）。第1回で既にデビューライブ受賞済みの2名を登録する（対象者シートには
# 元々含まれていない想定＝実データと同じ挙動）。他競技・他ステージの受賞行も混ぜてフィルタを確認する。
ws_win = wb.create_sheet('受賞者')
ws_win.append(['No.', '受賞区分', '受賞した競技名', '賞', '受賞者', '所属', '投稿タイトル・補足'])
ws_win.append([1, '標準アワード', 'デビューライブ', 'ゴールド', '山本 太郎さん', 'ダミー本部', 'ダミー投稿タイトルA'])
ws_win.append([2, '標準アワード', 'デビューライブ', 'ブロンズ', '鈴木 花子さん', 'ダミー本部', 'ダミー投稿タイトルB'])
ws_win.append([3, '標準アワード', 'アンコール', 'シルバー', '山本 太郎さん', 'ダミー本部', 'デビューライブより横断受賞'])

path = os.path.join(HERE, 'master_target_sheet.xlsx')
wb.save(path)
print('生成:', path)
print('対象者シート件数（除外3件含む）:', len(rows), '／ 全件版シート件数:', len(rows) + 1)
print('受賞者シート：デビューライブ受賞済み2名（他競技1件は除外対象）')
