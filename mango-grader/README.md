...
│       ├── i18n.js           # 多言語対応エンジン（辞書ベース、フレームワーク不使用）
│       ├── i18n/ja.js        # 日本語（原文）
│       ├── i18n/en.js        # 英語（簡易機械翻訳）
│       ├── i18n/zh.js        # 中国語簡体字（簡易機械翻訳）
...
モデルカードが「撮影条件が変わると精度が保証されない」と明記しているため、
`calibrate_thresholds()`（撮影環境向けに色のしきい値 `a_thr`/`h_thr` を選び直す機能）を
**アプリ内でも実行できるように移植しました**。設定 → 「⚙ 較正モード（スタッフ向け）」から：
...
日本語・英語・中国語（簡体字）に対応しています。ヘッダー右上のセレクトボックスで
切り替えられ、初回はブラウザの言語設定から自動選択されます（対応外の言語なら
日本語）。選択は端末のlocalStorageに保存され、次回も引き継がれます。

- `public/js/i18n.js` — 辞書ベースの最小限のi18nエンジン（フレームワーク不使用）。
  静的なテキストは `data-i18n="キー"` 属性（`index.html`）、動的に生成する文言は
  `t('キー', {変数})` （`app.js`）で参照します
- `public/js/i18n/ja.js` / `en.js` / `zh.js` — 言語ごとに独立したファイル。
  **日本語（`ja.js`）が原文（正）**で、英語・中国語は**簡易的な機械翻訳ベース**です。
  3ファイルとも同じキー集合を持つ必要があり（欠けているキーがあると日本語に
  フォールバックします）、専門の翻訳に差し替える場合は該当ファイルの値だけを
  書き換えれば反映されます
- 判定結果側（`model.js`）のエラー・注記は、翻訳キーの代わりに安定したコード
  （例: `no_fruit_detected`）を返すようにしてあります。UI側（`app.js`）がそのコードを
  現在の言語に翻訳するので、`/api/log` に送られる記録も**言語に依存しない一貫した
  コード**のまま残ります

**現状の制約**: `manifest.webmanifest`（ホーム画面のアプリ名など）とサーバー側の
`/api/*` のエラーメッセージは、まだ日本語固定です。
...
- **JS移植とは別に、7枚とも学習データの分布外だったことも観察されました。**
  `vr_whole` が0.001〜19.2%と学習データの非ゼロ最小値（20%台）を下回り、
  A判定は1枚も出ませんでした。ただし**この7枚には人間の正解等級が無く、
  かつ `mango_spoil` という傷んだ果実の見本セットとして提供されたもの**なので、
  較正のズレなのか単に低着色の個体なのかは**区別できていません**。
  切り分けには正解等級付きサンプルが必要です。詳細は
